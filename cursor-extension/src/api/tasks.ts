...
export interface Task {
    ...
    label_config?: any;  // Label Studio configuration for rendering the form
    data: any;
...
export interface TaskAssignment {
    id: number;
    task: number;  // Task ID only
    task_data?: Task;  // Full task object (from backend serializer)
    annotator: number;
    ...
    feedback?: string;  // Added missing feedback field
}
...
export class TaskManager {
    ...
    async getAvailableProjects(): Promise<Project[]> {
        try {
            ...
            const response = await client.get('/labelstudio/projects/published_projects/');
    ...
    async getAvailableTasks(): Promise<Task[]> {
        try {
            ...
            const response = await client.get('/tasks/', {
                params: {
                    status: 'available',
                    page_size: 1000  // Get all tasks in one request
                }
            });
    ...
    async claimTaskFromProject(projectId: number): Promise<TaskAssignment> {
        const client = this.authManager.getApiClient();
        // First, get an available task from this project
        const tasksResponse = await client.get('/tasks/', {
            params: {
                status: 'available',
                project: projectId,
                page_size: 1  // We only need one task
            }
        });

        const tasks = tasksResponse.data.results || tasksResponse.data;
        if (!tasks || tasks.length === 0) {
            throw new Error('No available tasks in this project');
        }

        // Claim the first available task
        return this.claimTask(tasks[0].id);
    }
    ...
    async refreshTasks(): Promise<void> {
        ...
        await this.storage.set('availableProjects', projects);
